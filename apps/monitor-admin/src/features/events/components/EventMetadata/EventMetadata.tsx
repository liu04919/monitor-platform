...
export function EventMetadata({ event }: { event: EventDetail }) {
  return (
    <section className={styles.card}>
      <header><h2>事件信息</h2><span>Schema v{event.schemaVersion}</span></header>
      <dl className={styles.grid}>
        ...
        <MetadataItem label="传输方式"><span className={`${styles.transport} ${event.sendType === 'beacon' ? styles.beacon : ''}`}>{event.sendType}</span></MetadataItem>

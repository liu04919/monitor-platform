...
var eventTypesByCategory = map[Category]map[string]struct{}{
	CategoryError: {
		"js_error":            {},
		"unhandled_rejection": {},
		"cors_error":          {},
...
func ValidateBatch(batch Batch) error {
	...
	if batch.SendType != SendTypeFetch && batch.SendType != SendTypeBeacon {
		return invalid("sendType", "must be fetch or beacon")
	}

	for index, event := range batch.Events {
		if err := validateEvent(event, index); err != nil {
			return err
		}
	}
...
func validateEvent(event Event, index int) error {
	...
	if event.Category == CategoryError || event.Category == CategoryStability {
		if event.Level == nil {
			return invalid(prefix+".level", "is required for error and stability events")
		}
		if *event.Level != LevelError && *event.Level != LevelWarning {
...
func validateBreadcrumb(breadcrumb Breadcrumb, eventPrefix string, index int) error {
	...
	case BreadcrumbCategoryClick,
		BreadcrumbCategoryNavigation,
		BreadcrumbCategoryHTTP,
		BreadcrumbCategoryConsole,
		BreadcrumbCategoryCustom:
	default:
		return invalid(prefix+".category", "is not supported")
	}

...
func (h *Handler) decodeCredentials(c *gin.Context) (credentialsRequest, bool) {
	...
	if !response.IsJSONContentType(c.GetHeader("Content-Type")) {
		response.WriteError(c, http.StatusUnsupportedMediaType, "UNSUPPORTED_MEDIA_TYPE", "Content-Type must be application/json", nil)
...
func writeDecodeError(c *gin.Context, err error) {
	...
	if errors.As(err, &maxBytesError) {
		response.WriteError(c, http.StatusRequestEntityTooLarge, "PAYLOAD_TOO_LARGE", "request body must not exceed 4 KiB", nil)
		return
	}

	response.WriteError(c, http.StatusBadRequest, "MALFORMED_JSON", "request body must contain exactly one valid credentials JSON value", nil)
}

...
func TestEventDetailHandlerMapsErrors(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "invalid event ID", err: eventdomain.ErrInvalidEventID, wantStatus: http.StatusBadRequest, wantCode: "INVALID_PATH", wantField: "eventId"},
		{name: "not found", err: eventdomain.ErrEventNotFound, wantStatus: http.StatusNotFound, wantCode: "EVENT_NOT_FOUND"},
		{name: "project not found", err: eventdomain.ErrProjectNotFound, wantStatus: http.StatusNotFound, wantCode: "PROJECT_NOT_FOUND"},
